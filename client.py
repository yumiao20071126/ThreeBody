...
from ThreeBody_main import DrawImage0
import cv2
    ...
    try:
        while True:
            ...
            if data:
                try:
                    # 反序列化对象
                    received_all = pickle.loads(data)
                    if isinstance(received_all, All):
                        print(f"Deserialized: x={received_all.stars[0].x}, y={received_all.stars[0].y}")
                    else:
                        print('收到未知对象:', received_all)
                    # frame0=DrawImage0(received_all)
                    # cv2.imshow('Moving Dot', frame0)
